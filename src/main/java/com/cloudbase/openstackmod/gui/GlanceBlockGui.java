...
public class GlanceBlockGui extends GuiContainer {
	private int option;
	private GuiButton buttonLeft;
	private GuiButton buttonRight;
	private GuiButton buttonChoose;
	private GlanceBlockEntity te;
	public GlanceBlockGui(IInventory playerInv, GlanceBlockEntity te) {
		super(new GlanceBlockGuiContainer(playerInv, te));
	...
	protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
	    ...
	    this.mc.getTextureManager().bindTexture(new ResourceLocation("openstackmod:textures/gui/container/glanceblockgui.png"));
	    this.drawTexturedModalRect(this.guiLeft, this.guiTop, 0, 0, this.xSize, this.ySize);
	}
	
	@Override
	protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
		String display = OpenStackConnector.getGlanceImages().split("&")[this.option].split(";")[0];
	...
	protected void actionPerformed(GuiButton button) throws IOException{
		if(button == this.buttonLeft){
			this.option--;
			if(this.option<0)
				this.option = OpenStackConnector.getGlanceImages().split("&").length-1;
		}
		if(button == this.buttonRight){
			this.option++;
			if(this.option>=OpenStackConnector.getGlanceImages().split("&").length)
				this.option = 0;
				
		}
		if(button == this.buttonChoose){
			te.convertImage(OpenStackConnector.getGlanceImages().split("&")[this.option]);
