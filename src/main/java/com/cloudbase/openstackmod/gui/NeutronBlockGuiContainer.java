...
public class NeutronBlockGuiContainer extends Container {
	private NeutronBlockEntity te;
	
	public NeutronBlockGuiContainer(IInventory playerInv, NeutronBlockEntity te){
		this.te = te;
		
		this.addSlotToContainer(new SingleObjectSlot(te, 0, 23, 12, BoardItem.instance, 1));
		this.addSlotToContainer(new SingleObjectSlot(te, 1, 144,13, ChipItem.instance, 1));
		this.addSlotToContainer(new SingleObjectSlot(te, 2, 80, 24, NICItem.instance, 1));
		//player inventory
    ...
    {
    	te.createNIC(OpenStackConnector.getNeutronNICs().split("&")[id]);
    	return true;
    }
}
